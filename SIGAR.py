...
parser.add_argument('--NM_threshould',default=4,type=int,help='The minimum number of nonmatched nucleotides in bwa alignment. default: %(default)s,type:%(type)s')
...
parser.add_argument('--BWA_other_args',default="",help='Other BWA parameters except the default ones (default command: bwa mem -t 1 genome_index reads.fq). e.g. -a. Note BWA is used to align split reads to MAC genome, adding parameters here could change the SIGAR results. type:%(type)s')
...
parser.add_argument('--Bowtie2_other_args',default="",help='Other Bowtie2 parameters except the default ones (--threads,-U,-x,-S) e.g.: -a --fast. Note bowtie2 is only used to remove reads with end-to-end alignment to MAC genome.')
